...
- ナビ（🚩）: 押すと目的地選択モード（中央の十字）。`←↑↓→`で地図を動かし決定で目的地確定 → 現在地からの徒歩ルートを描画し、上部に次の曲がり角を表示。歩くと案内が更新。もう一度🚩でナビ解除
  - ※ Google Cloud で **Directions API を有効化**し、APIキーの「APIの制限」に **Directions API** を追加すること
